  ...
  const CustomDay = ({ date, activeModifiers, displayMonth, ...props }) => {
    ...
    if (dayData) {
      return (
        <div onClick={() => sendDataToParent(dayData)} {...props} className={`flex flex-col items-center justify-center h-full w-full relative ${activeModifiers.selected? "bg-primary" : "bg-accent"} rounded-md`}>
          ...
          <div onClick={() => handleOpenDialog(dayData)} className="absolute top-1 right-0 text-xs px-2">
            <FontAwesomeIcon className='hidden' size='sm' icon={faCheck} />
          </div>
      </div>
      );
    }
    ...
    return (
      <div onClick={() => sendDataToParent(null)} {...props} className={`day-cell flex h-full w-full items-center justify-center`}>
        ...
        {activeModifiers.today? <div className="text-xs absolute top-11 right-4">Today</div> : <></>}
      </div>
    );
  };
  ...
  return (
    ...
    <DayPicker
      ...
      classNames={{
        ...
        cell: "rounded-md h-11 w-11 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day: cn(
...
ProCalendar.displayName = "Calendar"
